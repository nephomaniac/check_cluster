...
class AWSCollector:
    ...
    def __init__(self, region: str = None, max_retries: int = 3, retry_delay: int = 1, debug: bool = False, request_tracker=None):
        ...
        self.servicequotas = session.client('service-quotas', region_name=self.region, **client_kwargs)
    ...
    def list_service_quotas(self, service_code: str) -> Dict:
        """List service quotas for a specific AWS service"""
        print(f"aws service-quotas list-service-quotas --service-code {service_code} --output json")
        return self._tracked_request(
            service='service-quotas',
            operation='list_service_quotas',
            func=self.servicequotas.list_service_quotas,
...
class ClusterDataCollector:
    ...
    def __init__(self, cluster_id: str, work_dir: str = ".", region: str = None,
                 ...
                 force_update: bool = False, debug: bool = False, resources: list = None):
    ...
    def _should_collect(self, resource_type: str) -> bool:
        """Check if a resource type should be collected based on --resources argument."""
    ...
    def run(self):
        ...
        # Get AWS resources based on --resources argument
        # VPC and network infrastructure (required for most other resources)
        if self._should_collect('vpc'):
        ...
        if self._should_collect('quotas'):
            self._get_service_quotas()

        # EC2 instances
        if self._should_collect('ec2'):
            self._get_ec2_instance_info()

        # CloudTrail logs
        if self._should_collect('cloudtrail'):
            self._get_cloud_trail_logs()

        # Route53 DNS info
        if self._should_collect('route53'):
            self._get_route53_info()

        # Security groups
        if self._should_collect('security-groups'):
            self._get_security_groups_info()

        # Load balancers
        if self._should_collect('load-balancers'):
    ...
    def _get_vpc_endpoint_service_info(self):
        ...
        This collects information about the VPC endpoint SERVICE that allows customer
        VPCs to connect to the cluster's API. This is SEPARATE from VPC endpoints
        for AWS services (S3, EC2, ELB, STS) that the cluster needs to function.

        Two types of endpoints in PrivateLink:
        1. VPC Endpoint SERVICE (customer → cluster API) - collected here
        2. VPC Endpoints for AWS services (cluster → AWS) - collected in _get_additional_infrastructure
        """
        Colors.hdr("Getting VPC endpoint service info (customer → cluster connectivity)")

        if not self.private_link:
            return

        Colors.blue("PrivateLink cluster detected - fetching VPC endpoint service configuration")
        ...
        if Path(vpc_epsrv_file).exists():
            ...
            try:
                ...
                if response.get('ServiceConfigurations'):
                    svc = response['ServiceConfigurations'][0]
                    Colors.green(f"VPC Endpoint Service: {svc.get('ServiceId')} ({svc.get('ServiceState')})")
            except Exception as e:
        ...
        if Path(vpc_epsrv_file).exists():
            if Path(vpc_ep_conn_file).exists():
                ...
                with open(vpc_epsrv_file) as f:
                    ...
                    if service_id:
                        Colors.blue(f"Service ID: {service_id}")
                        Colors.blue("Fetching customer VPC endpoint connections...")
                        try:
                            ...
                            connections = response.get('VpcEndpointConnections', [])
                            if connections:
                                Colors.green(f"Found {len(connections)} customer VPC endpoint connection(s):")
                                for conn in connections:
                                    vpce_id = conn.get('VpcEndpointId')
                                    vpce_state = conn.get('VpcEndpointState')
                                    vpce_owner = conn.get('VpcEndpointOwner')
                                    Colors.blue(f"  - {vpce_id} (owner: {vpce_owner}, state: {vpce_state})")
                                    if vpce_owner != self.aws.account_id:
                                        Colors.yellow(f"    Note: Endpoint is in a different AWS account ({vpce_owner})")
                            else:
                                Colors.yellow("No customer VPC endpoint connections found")
                                Colors.yellow("This is normal during installation - customers connect after cluster is ready")
                        except Exception as e:
    ...
    def _get_subnets_and_vpcs_from_cluster_json(self):
        ...
        # Track VPC IDs discovered from subnets (store as instance variable for later use)
        self.discovered_vpc_ids = set()
        fetched_subnets = []
            ...
            if Path(subnet_file).exists():
                ...
                with open(subnet_file) as f:
                    ...
                    if 'Subnets' in subnet_data and subnet_data['Subnets']:
                        ...
                        if vpc_id:
                            self.discovered_vpc_ids.add(vpc_id)
        ...
        if self.discovered_vpc_ids:
            Colors.blue(f"Discovered {len(self.discovered_vpc_ids)} VPC(s): {', '.join(self.discovered_vpc_ids)}")

            for vpc_id in self.discovered_vpc_ids:
    ...
    def _get_service_quotas(self):
        """
        Fetch AWS service quotas for ROSA prerequisite validation.

        Collects service quotas for:
        - EC2: vCPU quotas for instance creation
        - VPC: Network infrastructure limits
        - ELB: Load balancer quotas
        - EBS: Storage quotas
        """
        Colors.hdr("Getting AWS Service Quotas")

        quotas_file = f"{self.file_prefix}_service_quotas.json"

        if Path(quotas_file).exists():
            Colors.green(f"Using existing service quotas file: {quotas_file}")
            return

        Colors.blue("Fetching service quotas from AWS...")
        Colors.yellow("Note: This may take a minute as we query multiple services")

        # Service codes for ROSA-relevant AWS services
        # Reference: https://docs.aws.amazon.com/general/latest/gr/aws-service-information.html
        service_codes = {
            'ec2': 'EC2',
            'vpc': 'VPC',
        ...
        for service_code, service_name in service_codes.items():
            try:
                Colors.blue(f"  Fetching {service_name} quotas...")
                response = self.aws.list_service_quotas(service_code)
                all_quotas[service_code] = response

                quota_count = len(response.get('Quotas', []))
                Colors.green(f"  ✓ {service_name}: {quota_count} quotas")

            except Exception as e:
                error_str = str(e)
                Colors.perr(f"  ✗ Failed to fetch {service_name} quotas: {error_str}")
                failed_services.append(service_name)

                # Check if it's a permission error
                if 'AccessDenied' in error_str or 'UnauthorizedOperation' in error_str:
                    Colors.yellow(f"     Missing permission: servicequotas:ListServiceQuotas for {service_code}")
                    Colors.yellow(f"     Service quota validation tests will be skipped for {service_name}")

        # Save what we have, even if some services failed
        if all_quotas:
            with open(quotas_file, 'w') as f:
                json.dump(all_quotas, f, indent=2, default=str)
            Colors.green(f"Saved service quotas to {quotas_file}")

            if failed_services:
                Colors.yellow(f"\nPartial data saved. Failed services: {', '.join(failed_services)}")
        else:
            Colors.perr("No service quota data collected - all services failed")
            Colors.yellow("Service quota validation tests will be skipped")
    ...
    def _get_additional_infrastructure(self):
        ...
        # VPC Endpoints (cluster → AWS services)
        # For PrivateLink clusters, we need ALL VPC endpoints in the VPC, not just tagged ones
        # AWS service endpoints (S3, EC2, ELB, STS) may not have cluster tags
        # NOTE: These are DIFFERENT from VPC endpoint service connections (customer → cluster)
        ...
        if Path(vpc_endpoints_file).exists():
            ...
            if self.private_link:
                Colors.blue("Fetching VPC endpoints for AWS services (cluster → S3, EC2, ELB, STS)...")
            else:
                Colors.blue("Fetching VPC endpoints from AWS...")
            try:
                # Try to get VPC ID from previously loaded VPC data
                vpc_id_filter = None
                if hasattr(self, 'vpc_id') and self.vpc_id:
                    vpc_id_filter = [{'Name': 'vpc-id', 'Values': [self.vpc_id]}]
                elif hasattr(self, 'discovered_vpc_ids') and self.discovered_vpc_ids:
                    # Use the first discovered VPC ID
                    vpc_id_filter = [{'Name': 'vpc-id', 'Values': [list(self.discovered_vpc_ids)[0]]}]

                # Collect endpoints - prefer VPC filter, fallback to tag filter
                if vpc_id_filter:
                    Colors.blue(f"Fetching VPC endpoints by VPC ID filter...")
                    response = self.aws.describe_vpc_endpoints(filters=vpc_id_filter)
                else:
                    Colors.blue(f"Fetching VPC endpoints by tag filter (VPC ID not available)...")
                ...
                if endpoint_count > 0:
                    Colors.green(f"Found {endpoint_count} VPC endpoint(s) for AWS services")
                    # Show summary of what we found
                    endpoints = response.get('VpcEndpoints', [])
                    gateway_eps = [ep for ep in endpoints if ep.get('VpcEndpointType') == 'Gateway']
                    interface_eps = [ep for ep in endpoints if ep.get('VpcEndpointType') == 'Interface']
                    if gateway_eps:
                        Colors.blue(f"  Gateway endpoints: {len(gateway_eps)}")
                    if interface_eps:
                        Colors.blue(f"  Interface endpoints: {len(interface_eps)}")
                else:
                    Colors.yellow("No VPC endpoints found in cluster VPC")
                    if self.private_link:
                        Colors.yellow("WARNING: PrivateLink clusters require VPC endpoints for AWS services!")
                        Colors.yellow("Required: S3 (gateway), EC2, ELB, STS (interface)")
            except Exception as e:
